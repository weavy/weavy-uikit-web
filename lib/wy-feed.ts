...
import type { PostQueryFilterProps, PostQueryFilterType, PostType } from "./types/posts.types";
...
/**
 * Weavy posts component to render a feed of posts with an editor and comments as seen on many social networks. The component includes a fully featured post editor with built in support for markdown, code formatting, images, file attachments, cloud attachments, mentions, links, embeds, and polls.
 *
 ...
 *
 * The feed is populated from a single post app or from multiple specified post apps by specifying one or multiple post app identifiers using the `uid` attribute. Specifying only one uid allows automatic creation of a [corresponding post app](https://www.weavy.com/docs/concepts#app) on the server the first time it's accessed. When using multiple uid:s, all the specified post apps must exist prior to loading the feed.
 *
 * > To show a combined feed of _all_ post apps available to the user, it's recommended to use the [`<wy-posts>`](./wy-posts.ts) component, which has additional configuration options, together with a _separate_ [`<wy-post-editor>`](./wy-posts-editor.ts) component.
 *
 ...
 * **Component Layout**
 *
 * The component has the editor in the top. Below the editor is the post feed with the optional search field in the top of the list. The editor and the feed can alternatively be used and configured separately by using the [`<wy-post-editor>`](./wy-post-editor.ts) component and the [`<wy-posts>`](./wy-posts) component.
 *
 ...
 * - [`<wy-post-form>`](./components/wy-post-form.ts)
 ...
 * @csspart wy-feed - Root layout container.
 ...
 * ```
 */
@customElement("wy-feed")
@localized()
export class WyFeed extends WeavyAppComponent implements PostQueryFilterProps {
  ...
  /** @internal */
  protected theme = new ThemeController(this, WyFeed.styles);

  /**
   * Reference to the post feed
   *
   * @internal
   */
  ...
  override render() {
    return html`
      ...
      <wy-container part="wy-feed" padded outer gap="xl">
        <wy-post-form .placeholder=${this.placeholder}></wy-post-form>
        ${this.search
          ? html`
